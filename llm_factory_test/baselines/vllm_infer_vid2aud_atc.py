...
# -*- coding: utf-8 -*-
"""
vLLM LLMEngine offline inference script

Usage example:
CUDA_VISIBLE_DEVICES=0 python vllm_infer_sort_atc.py

Notes:
- This script uses EngineArgs/LLMEngine for low-level control (add_request + step loop).
- It keeps your tokenizer.apply_chat_template(...) to build the chat prompt.
"""
import argparse
import json
import os
    ...
    try:
        while queue or engine.has_unfinished_requests():
            ...
            for ro in request_outputs:
                ...
                # 放入缓冲（不立即写）
                rec = dict(all_data[idx])
                rec["model_generate"] = gen_text
                buffer[idx] = rec
...
def parse_args():
    ...
    parser = FlexibleArgumentParser(description="vLLM LLMEngine offline inference wrapper")
    parser = EngineArgs.add_cli_args(parser)

    # 脚本自身参数（与原脚本保持兼容）
    parser.add_argument("--model_path", type=str, default="/data/phd/qinsizhong/llm_factory_test/saves/qwen-8b-sft-1028-aud/checkpoint-6500",
                        help="(optional) alias to set --model for EngineArgs")
    parser.add_argument("--src_path", type=str,default='/data/phd/miltonzhou/sft/data_preprocess/sft_1028_eval.json',
                        help="输入 jsonl 文件（每行一条 json）")
    parser.add_argument("--result_path", type=str, default="/data/phd/qinsizhong/llm_factory_test/baselines/results/__vid2aud_atc_1028.jsonl",
                        help="输出 jsonl 文件（可选）")
