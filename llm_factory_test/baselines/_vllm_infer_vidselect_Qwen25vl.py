...
# -*- coding: utf-8 -*-
"""
vLLM LLMEngine offline inference (Qwen2.5-VL) — select/sort （Picture k + images, hide frame_id）

改动要点：
- 从输入记录透传 meta: sample_id / task / ad_key 到输出最前面，便于与 manifest 对齐检索；
- meta 不注入 prompt，不影响模型推理；
- 输出为精简对象，仅包含：sample_id, task, ad_key, conversations, system, tools, model_generate。
"""
...
def _extract_human_value(rec: Dict) -> str:
    """从 conversations 里取第一条 human/user 的 value；找不到则退回第 0 条。"""
...
def _extract_frames_and_rewrite(text: str) -> Tuple[List[str], str]:
    """
    从文本中提取 [frame_id]，并将其依次替换为 "Picture 1", "Picture 2", ...
    返回 (frame_ids, new_text)
    """
    frame_ids: List[str] = []
...
def prepare_prompts_from_frames(
    ...
    for idx, rec in enumerate(records):
        ...
        # 多模态 content：先所有图片，再文本（文本已是 Picture k）
        ...
        request_id = str(idx)
        items.append((request_id, prompt_text, idx, image_paths))
...
def process_with_engine(
    ...
    """
    逐条将请求送入 vLLM Engine，并把生成结果写回文件。
    输出使用 build_output_record（meta 透传且不进入 prompt）。
    """
    ...
    try:
        while queue or engine.has_unfinished_requests():
            ...
            for ro in request_outputs:
                ...
                out_obj = build_output_record(base_records[idx], gen_text)
...
def parse_args():
    parser = FlexibleArgumentParser(
        description="vLLM LLMEngine offline inference (Qwen2.5-VL, use Picture k + images, hide frame_id, preserve meta)."
    )
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="/data/phd/hf_models/Qwen2.5-VL-32B-Instruct",
    )
    parser.add_argument(
        "--prompts_path",
        type=str,
        default="/data/phd/miltonzhou/sft/data_preprocess/T_baseline_mm_vidselect.json",
        help="JSON/JSONL; 每条记录包含 `conversations`，human 文本中含有 [frame_id] 占位符。",
    )
    parser.add_argument(
        "--frames_dir",
        type=str,
        default="/data/phd/miltonzhou/sft/data_preprocess/filter_data/downloaded_frames",
        help="帧图像目录，文件名为 {frame_id}.jpg。",
    )
    parser.add_argument(
        "--result_path",
        type=str,
        default="/data/phd/qinsizhong/llm_factory_test/baselines/results/F___vidselect_Qwen25vl_32B_222.jsonl",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="是否让 tokenizer 自动为图片加入 'Picture k:' 标记。",
    )
...
def main():
    ...
    engine_args.max_model_len = 65536
